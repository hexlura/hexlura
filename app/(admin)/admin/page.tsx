...
export default async function AdminDashboardPage() {
    ...
    // KPI: Tickets sold today — confirmed bookings only, no double-filter on booking_items.created_at
    ...
    const { data: confirmedTodayBookings } = await adminClient
        ...
        .select('id')
        .eq('status', 'confirmed')
        .or(`confirmed_at.gte.${todayMidnight.toISOString()},and(confirmed_at.is.null,created_at.gte.${todayMidnight.toISOString()})`)

    const confirmedTodayIds = (confirmedTodayBookings || []).map((b: { id: string }) => b.id)
    ...
    if (confirmedTodayIds.length > 0) {
        const { data: todayItems } = await adminClient
            .from('booking_items')
            .select('quantity')
            .in('booking_id', confirmedTodayIds)
        ticketsSoldToday = (todayItems || []).reduce((s: number, i: { quantity: number }) => s + (i.quantity || 0), 0)
    }
