...
export async function POST(request: Request) {
    ...
    if (event.type === 'payment_intent.succeeded') {
        ...
        // Promoter earnings ledger row for paid bookings
        if (promoterId && promoterCommissionPercent !== null && promoterCommissionPence !== null) {
            ...
            if (eventOrg?.organiser_id) {
                try {
                    await supabase.from('promoter_earnings').insert({
                        promoter_id: promoterId,
                        booking_id: booking.id,
                        event_id: eventId,
                        organiser_id: eventOrg.organiser_id,
                        ticket_subtotal_pence: ticketSubtotalPence - discountPence,
                        commission_percent: promoterCommissionPercent,
                        commission_pence: promoterCommissionPence,
                        status: 'pending',
                    })
                } catch (err) {
