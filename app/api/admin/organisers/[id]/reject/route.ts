...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
    ...
    const { data: org } = await supabase.from('organiser_profiles').select('user_id').eq('id', params.id).single()

    // Delete organiser profile
    await supabase.from('organiser_profiles').delete().eq('id', params.id)

    // Set profile role back to user
    if (org?.user_id) {
        await supabase.from('profiles').update({ role: 'user' }).eq('id', org.user_id)
    }
