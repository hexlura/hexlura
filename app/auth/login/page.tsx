...
function LoginContent() {
    ...
    async function handleSubmit(formData: FormData) {
        ...
        if (result?.error) {
            ...
            // If a booking-flow next param is present, honour it over role redirect
            // Ignore generic /account or / values — use the role-based destination instead
            if (next && next !== '/account' && next !== '/') {
                router.push(next)
            } else {
                // Use role-based destination returned by server (/admin, /organiser, /account)
