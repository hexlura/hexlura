...
export default async function OrganiserPayoutsPage() {
    ...
    return (
        <div className="max-w-5xl">
            ...
            {/* Identity verification banner — shows whenever not verified */}
            {!identityVerified && (
                <div className="bg-gold/10 border border-gold/30 rounded-none p-4 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                    ...
                    <a
                        ...
                        className="px-4 py-2 bg-gold text-black text-sm rounded-sm font-medium hover:bg-yellow-400 transition-colors text-center whitespace-nowrap"
                    >
                        {organiser.identity_status === 'processing' ? 'Check status' : 'Verify Identity'}
                    </a>
                </div>
            )}
            ...
            <div className="bg-card border border-border rounded-none p-6">
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {payouts.length === 0 && (
                            <tr><td colSpan={5} className="text-center text-muted text-xs py-12">No payouts yet</td></tr>
                        )}
                        {payouts.map(p => (
                            <tr key={p.id} className="border-b border-border/50">
                                <td className="py-3 pr-4 text-text text-xs max-w-[200px] truncate">
                                    ...
                                    {p.status === 'paid' && p.reference && (
                                        <div className="text-[10px] text-muted font-mono mt-0.5 truncate">Ref: {p.reference}</div>
                                    )}
                                </td>
                                ...
                                <td className="py-3 pr-4 text-muted text-xs">{getDate(p)}</td>
                                <td className="py-3">
                                    <a
