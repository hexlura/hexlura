...
export default async function OrganiserDashboardPage() {
    ...
    const { data: organiser } = await supabase
        .from('organiser_profiles').select('id, org_name').eq('user_id', user.id).single()
    ...
    const { data: allBookings } = eventIds.length
        ? await supabase
            ...
            .in("event_id", eventIds).eq("status", "confirmed").order("created_at", { ascending: false })
        : { data: [] }
    const bookings = (allBookings || []) as {
        id: string; booking_ref: string; ticket_subtotal_pence: number | null;
        created_at: string; event: { title?: string } | null
    }[]

    const bookingIds = bookings.map(b => b.id)
    const { data: allItems } = bookingIds.length
        ? await supabase.from("booking_items").select("booking_id, quantity, attendee_name").in("booking_id", bookingIds)
        : { data: [] }
    const items = (allItems || []) as { booking_id: string; quantity: number; attendee_name: string | null }[]
    ...
    const { data: pendingPayouts } = await supabase
        .from("payouts").select("net_pence").eq("organiser_id", organiserId).eq("status", "pending")
