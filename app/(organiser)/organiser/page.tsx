...
export default async function OrganiserDashboardPage({ searchParams }: PageProps) {
    ...
    type Kpi =
        | { kind: 'single'; label: string; value: string; sub: string }
        | { kind: 'split'; label: string; primary: { value: string; sub: string }; secondary: { value: string; sub: string } }
    ...
    const kpis: Kpi[] = [
        ...
        { kind: 'single', label: 'Upcoming Events', value: String(upcoming.length), sub: upcoming[0] ? 'Next: ' + fmt(upcoming[0].start_at) : 'None scheduled' },
        { kind: 'single', label: 'Payout Balance', value: formatPence(payoutPence), sub: payoutPence > 0 ? 'Available for withdrawal' : 'Released after event cooldown' },
    ]
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {recentBookings.length === 0 && (
                                <tr><td colSpan={5} className="text-center text-muted text-xs py-8">
