        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="flex flex-col lg:flex-row gap-12">
                ...
                <div className="w-full lg:w-3/5 flex flex-col gap-10">
                    ...
                    {/* Key Info Row */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-muted/10 rounded-xl border border-border/50">
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="w-full h-64 bg-muted rounded-xl flex flex-col items-center justify-center border border-border text-muted-foreground">
                    ...
                    {/* Organiser Profile */}
                    {event.organiser && (
                        <section className="mt-8 p-6 bg-card border rounded-xl flex items-center justify-between shadow-sm">
                            <div className="flex items-center gap-4">
                                <div className="w-16 h-16 rounded-full bg-muted border overflow-hidden relative">
                                    {event.organiser.logo_url ? <Image src={event.organiser.logo_url} alt="" fill className="object-cover" /> : <div className="w-full h-full flex items-center justify-center font-bold text-xl">{event.organiser.org_name.charAt(0)}</div>}
                                </div>
                                <div>
                                    <p className="text-sm text-muted-foreground mb-1">Organised by</p>
                                    <h4 className="text-xl font-bold">{event.organiser.org_name}</h4>
                                </div>
                            </div>
                            <Link href={`/organisers/${event.organiser.slug}`} className="px-4 py-2 border rounded-full font-medium hover:bg-muted transition-colors text-sm">
                                View Profile
                            </Link>
                        </section>
                    )}
                        ...
                        {reviews.length > 0 ? (
                            <div className="space-y-4">
                                {reviews.map((review: Review) => (
                                    <div key={review.id} className="p-4 border rounded-lg bg-card text-sm">
