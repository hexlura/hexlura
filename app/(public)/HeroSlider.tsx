...
function EventSlide({ event, priority = false }: { event: FeaturedEvent; priority?: boolean }) {
        ...
        <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }}>
            ...
            <div className="hero-content" style={{ position: 'absolute', inset: 0, zIndex: 2, display: 'flex', alignItems: 'center', padding: '0 48px', gap: '40px' }}>
                ...
                <div className="hero-portrait" style={{
                    ...
                    {hasBanner ? (
                        <Image src={event.banner_url!} alt={event.title} fill priority={priority} sizes="200px" style={{ objectFit: 'cover' }} />
                    ) : (
...
function FomoSlide() {
    return (
        <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }}>
            {/* BG image — no priority: this slide is slot 5+ in the carousel,
                so it's never the LCP element on first paint. */}
            <Image
...
function renderSlide(slide: SlideData, priority = false) {
    switch (slide.type) {
        case 'brand': return <BrandSlide />
        case 'event': return <EventSlide event={slide.event} priority={priority} />
...
export function HeroSlider({ slides }: { slides: SlideData[] }) {
    ...
    // Index of the FIRST event slide in the deck — that one gets priority
    // so the browser preloads its banner image (the post-text LCP candidate).
    const firstEventSlideIndex = slides.findIndex(s => s.type === 'event')

    return (
        <div
            className="hero-slider"
            style={{ width: '100%', height: '520px', position: 'relative', overflow: 'hidden', background: '#0A0A0F', borderRadius: 0 }}
        ...
        >
            ...
            <div style={{ width: '100%', height: '100%', opacity: visible ? 1 : 0, transition: 'opacity 0.6s ease' }}>
                {renderSlide(slide, current === firstEventSlideIndex)}
            </div>
